...
func TestSQLite_DualWrite(t *testing.T) {
	s := newTestStore(t)
...
func TestSQLite_OutboxOperations(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	...
	// SQLite retrieval order might vary unless strictly ordered, but outbox_spans orders by created_at.
	// We'll iterate to check.
...
func TestSQLite_DeleteOutboxSpans_Chunking(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()

	// Ingest 1500 spans (which exceeds the 500 chunk limit and the typical 999 SQLite variable limit)
	...
	// Increment attempts (should chunk 500 at a time)
	err = s.IncrementOutboxAttempt(ctx, ids)
	require.NoError(t, err)

	// Delete them all (should chunk 500 at a time)
...
func TestSQLite_ConcurrentPruneAndWrite(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
